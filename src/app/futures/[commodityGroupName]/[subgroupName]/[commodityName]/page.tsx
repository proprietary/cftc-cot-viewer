                ...
                {markets.map(({ marketAndExchangeName, contractsSet }, jdx) => (
                    <div key={jdx} className="my-5">
                        <div className="text-lg py-2 inline-flex">
                            <div className="font-medium uppercase tracking-wide pr-2">
                                {marketAndExchangeName}
                            </div>
                            (
                            {
                                Object.values(contractsSet).flat(1).at(0)
                                    ?.contractMarketName
                            }
                            )
                        </div>
                        <ul className="ml-5">
                            {contractsSet[CFTCReportType.FinancialFutures].map(
                                (contract, idx) => (
                            ...
                            {contractsSet[CFTCReportType.Disaggregated].map(
                                (contract, idx) => (
                                    <li key={idx} className="list-disc">
                                        <Link
                                            key={idx}
                            ...
                            {contractsSet[CFTCReportType.Legacy].map(
                                (contract, idx) => (
                                    <li key={idx} className="list-disc">
                                        <Link
                                            key={idx}
...
export async function generateStaticParams({
    ...
    for (const [commodityGroupName, subtree] of contractsTree
        .selectTree({}, ['group', 'commoditySubgroupName', 'commodityName'])
        .node.entries()) {
