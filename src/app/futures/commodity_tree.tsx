        ...
        contracts.map((contract, contractIdx) => (
            <div key={contractIdx} className="ml-3">
                <Link
                    ...
                    href={`/futures/${allCapsToSlug(
                        contract.group!
                    )}/${allCapsToSlug(
                        contract.commoditySubgroupName!
                    )}/${allCapsToSlug(contract.commodityName!)}/${
        ...
        <div className="block m-2">
            ...
            <div className="my-3">
                <h3 className="text-lg font-bold block">
                    <Link
                        href={`/futures/${commodityGroupNameSlug}/${commoditySubgroupNameSlug}/${commodityNameSlug}`}
                    >
                        {commodityNameTitle}
                    </Link>
                </h3>
                {commodityTree[CFTCReportType.FinancialFutures].length > 0 && (
                    <div>
                        ...
                        {renderContractsLinks(
                            commodityTree[CFTCReportType.FinancialFutures]
                        )}
                    </div>
                )}
                {commodityTree[CFTCReportType.Disaggregated].length > 0 && (
                    <div>
                        ...
                        {renderContractsLinks(
                            commodityTree[CFTCReportType.Disaggregated]
                        )}
                    </div>
                )}
                ...
                {commodityTree[CFTCReportType.Legacy].length > 0 && (
                    <div>
                        ...
                        {renderContractsLinks(
                            commodityTree[CFTCReportType.Legacy]
                        )}
                    </div>
                )}
            </div>
