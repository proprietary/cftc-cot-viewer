...
function extractRawNetPositioning<
    RptType extends
        | IFinancialFuturesCOTReport
        | IDisaggregatedFuturesCOTReport
        | ILegacyFuturesCOTReport,
>(
    ...
    traderCategories: ITraderCategory<RptType>[]
...
function extractZscoredPositioning<
    RptType extends
        | IFinancialFuturesCOTReport
        | IDisaggregatedFuturesCOTReport
        | ILegacyFuturesCOTReport,
...
const tooltipFormatters: Record<
    NormalizationMethod,
...
const yAxisLabels: Record<NormalizationMethod, (label: string) => string> = {
    [NormalizationMethod.StandardZscore]: (label) =>
        `${label} - σ (standard deviations)`,
...
export default function StandardizedCotOscillator({
    ...
    const [normalizationMethod, setNormalizationMethod] =
        React.useState<NormalizationMethod>(NormalizationMethod.StandardZscore)
    ...
    const computeSeries = React.useCallback((): BarSeriesOption[] => {
        ...
        for (const column of plottedColumns) {
            ...
            switch (normalizationMethod) {
                ...
                default:
                    throw new Error(
                        'unknown normalization method; this should be unreachable'
                    )
            }
            series.push({
                id: column.name,
            ...
            dataZoom: [
                {
                    ...
                    // ...dataZoomInner,
                    start:
                        rememberedDataZoom.current != null
                            ? rememberedDataZoom.current[0]
                            : (100 *
                                  Math.max(
                                      0,
                                      xAxisDates.length - defaultWeeksZoom
                                  )) /
                              xAxisDates.length,
                    end:
                        rememberedDataZoom.current != null
                            ? rememberedDataZoom.current[1]
                            : 100,
                },
            ],
            series: [
                ...computeSeries(),
                {
                    ...
                    yAxisIndex: 1,
                    data:
                        priceData != null ? priceData.map((x) => x.close) : [],
                },
            ],
            xAxis: [
                {
    ...
    React.useEffect(() => {
        ...
        if (legendSelected.current != null) {
            ...
            for (const [legendItemName, isSelected] of Object.entries(
                legendSelected.current
            )) {
                if (!isSelected)
                    ec?.dispatchAction({
    ...
    const handleChangeZsLookback = React.useCallback(
    ...
    const handleSetNormalizationMethod = React.useCallback(
        (ev: React.ChangeEvent<HTMLInputElement>) => {
        ...
        <div className="w-full">
            <div
                className={`cursor-pointer my-5 ${
                    showSettings ? 'stroke-gray-500' : 'stroke-inherit'
                }`}
            ...
            <div
                className={`m-2 flex flex-wrap items-center space-x-4 overflow-x-auto ${
                    showSettings ? '' : 'hidden'
                }`}
            >
                <label className="flex items-center space-x-2">
                    <input
                        type="radio"
                        value={NormalizationMethod.StandardZscore}
                        checked={
                            normalizationMethod ===
                            NormalizationMethod.StandardZscore
                        }
                ...
                <label className="flex items-center space-x-2">
                    <input
                        type="radio"
                        value={NormalizationMethod.RobustScaler}
                        checked={
                            normalizationMethod ===
                ...
                <label className="flex items-center space-x-2">
                    <input
                        type="radio"
                        value={NormalizationMethod.MinMaxScaler}
                        checked={
                            normalizationMethod ===
                ...
                <label className="flex items-center space-x-2">
                    <input
                        type="radio"
                        value={NormalizationMethod.QuantileTransformer}
                        checked={
                            normalizationMethod ===
            ...
            <div
                className={`m-2 flex items-center overflow-x-auto max-w-full ${
                    showSettings ? '' : 'hidden'
                }`}
            >
                <label className="flex flex-col w-3/4">
                    <div>
                        ...
                        <strong className="px-2">{lookback}</strong>
                    </div>
                    <input
                        type="range"
                        className="py-3"
