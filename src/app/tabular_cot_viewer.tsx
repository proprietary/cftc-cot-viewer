...
type FilteredCOTReportOnlyKeysToNumbers<
    T extends
        | IFinancialFuturesCOTReport
        | IDisaggregatedFuturesCOTReport
        | ILegacyFuturesCOTReport,
> = FilteredCOTReport<T>[keyof T]

export interface ITradersCategoryColumn<
    T extends
        | IFinancialFuturesCOTReport
        | IDisaggregatedFuturesCOTReport
        | ILegacyFuturesCOTReport,
...
export default function TabularCOTViewer<
    T extends
        | IFinancialFuturesCOTReport
        | IDisaggregatedFuturesCOTReport
        | ILegacyFuturesCOTReport,
>({
    ...
    const [currentReportsIdx, setReportsIdx] = React.useState<number>(
        reports.length - 1
    )
    React.useEffect(() => {
    ...
    const navNextPage = (ev: any) => {
        ev.preventDefault()
        setReportsIdx((prevReportsIdx) =>
            Math.min(reports.length - 1, prevReportsIdx + 1)
        )
    }
    ...
    return (
        <div className="overflow-x-auto mx-auto">
            <div className="flex-inline gap-5">
                ...
                <button
                    disabled={currentReportsIdx >= reports.length - 1}
                    onClick={navNextPage}
                    className="px-5"
                >
                    <svg
            ...
            <div>
                <table className="table-auto">
                    <caption>
                        {
                            reports.at(currentReportsIdx)
                                ?.market_and_exchange_names
                        }{' '}
                        week ending{' '}
                        {formatDateYYYYMMDD(
                            new Date(report?.timestamp ?? new Date().getTime())
                        )}{' '}
                        - Net Positions
                    </caption>
                    <thead>
                        <tr>
                            <th></th>
                            <th>
                                <abbr title="Longs minus Shorts">
                                    Net Position
                                </abbr>
                            </th>
                            <th>
                                <abbr title="Net position 1 week ago">
                                    1wk ago
                                </abbr>
                            </th>
                            <th>
                                <abbr title="Net position 4 weeks ago">
                                    4wk ago
                                </abbr>
                            </th>
                            <th>
                    ...
                    <tbody>
                        {reports.length > 0 &&
                            columns.map((column, colIdx) => {
                                ...
                                const past3M = newArrSlice(
                                    reports
                                        .slice(
                                            Math.max(0, currentReportsIdx - 12)
                                        )
                                        .map(
                                            (x) =>
                                                (x[column.longs] as number) -
                                                (x[column.shorts] as number)
                                        )
                                )
                                const past3MPercentile = quantileTransformAt(
                                    past3M,
                                    past3M.arr.length - 1,
                                    0,
                                    1
                                )
                                const past1Y = newArrSlice(
                                    reports
                                        .slice(
                                            Math.max(0, currentReportsIdx - 52)
                                        )
                                        .map(
                                            (x) =>
                                                (x[column.longs] as number) -
                                ...
                                const past1YPercentile = quantileTransformAt(
                                    past1Y,
                                    past1Y.arr.length - 1,
                                    0,
                                    1
                                )
                                const past2Y = newArrSlice(
                                    reports
                                        .slice(
                                            Math.max(0, currentReportsIdx - 104)
                                        )
                                        .map(
                                            (x) =>
                                                (x[column.longs] as number) -
                                ...
                                const past2YPercentile = quantileTransformAt(
                                    past2Y,
                                    past2Y.arr.length - 1,
                                    0,
                                    1
                                )
                                const past6M = newArrSlice(
                                    reports
                                        .slice(
                                            Math.max(0, currentReportsIdx - 26)
                                        )
                                        .map(
                                            (x) =>
                                                (x[column.longs] as number) -
                ...
                <table className="table-auto caption-top border-collapse min-w-full mt-8">
                    <caption>
                        ...
                        {formatDateYYYYMMDD(
                            new Date(
                                reports.at(currentReportsIdx)?.timestamp ?? 0
                            )
                        )}{' '}
                        - Full Report
                    </caption>
                    <thead>
                        ...
                        <tr>
                            ...
                            <th className="ml-8 text-right">Positions</th>
                            <th className="text-right">
                                <abbr title="Change in long positions from the previous week">
                                    Δ
                                </abbr>
                            ...
                            <th className="text-right">
                                <abbr title="Number of traders spreading">
                                    #
                                </abbr>
                            </th>
                        </tr>
                                ...
                                return (
                                    <tr key={colIdx}>
                                        ...
                                        <td className="font-mono text-right ml-8">
                                            {fmtThousandsSeparators(
                                                report[column.shorts] as number
                                            )}
                                        </td>
                                        <td className="font-mono text-right">
                                            {fmtThousandsSeparators(
                                                report[
                                                    column.changeInShorts
                                                ] as number
                                            )}
                                        </td>
                                        <td className="font-mono text-right">
