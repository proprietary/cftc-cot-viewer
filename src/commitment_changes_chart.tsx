...
export default function CommitmentChangesChart<
    T extends
        ...
        | IDisaggregatedFuturesCOTReport
        | ILegacyFuturesCOTReport,
>({ dataFrame, cols }: { dataFrame: Array<T>; cols: IDataFrameColumns<T>[] }) {
    ...
    const [nWeeksDelta, setNWeeksDelta] = React.useState<number>(
        DEFAULT_N_WEEKS_DELTA
    )
    const [posnMethod, setPosnMethod] =
        React.useState<PositioningAggregationMethod>(
            PositioningAggregationMethod.Net
        )
            ...
            for (let i = nWeeksDelta; i < dataFrame.length; ++i) {
                ...
                switch (posnMethod) {
                    case PositioningAggregationMethod.Net: {
                        data.push([
                            thisRow.timestamp,
                            (thisRow[longs] as number) -
                                (thisRow[shorts] as number) -
                                ((prevRow[longs] as number) -
                                    (prevRow[shorts] as number)),
                        ])
                        break
                    }
    ...
    const genOpt = React.useCallback((): ECOption => {
        return {
            ...
            title: {
                ...
                text:
                    dataFrame.length > 0
                        ? dataFrame.at(0)!.contract_market_name
                        : '',
                textStyle: {
                    fontSize: 10,
                },
            ...
            xAxis: cols.map((_, idx) => ({
                ...
                axisLabel: {
                    formatter: (value: any) =>
                        formatDateYYYYMMDD(new Date(value)),
                },
                gridIndex: idx,
            })),
    ...
    const optTypes = [
        PositioningAggregationMethod.Net,
        PositioningAggregationMethod.Longs,
        PositioningAggregationMethod.Shorts,
    ]
    return (
        <div className="my-2">
            <div className="flex items-center">
                <label className="w-full flex flex-col">
                    <div>
                        Change in position vs. <strong>{nWeeksDelta}</strong>{' '}
                        weeks ago:
            ...
            <div className="flex items-center">
                {optTypes.map((option, idx) => {
                    return (
                        <label
                            className="inline-flex items-center space-x-2 cursor-pointer px-3"
                            key={idx}
                        >
                            <input
                                type="radio"
