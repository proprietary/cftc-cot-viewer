...
export class CachingCFTCApi {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (dbReq != null) {
                ...
                dbReq.onupgradeneeded = function (ev: Event) {
                    ...
                    // TFF ObjectStore
                    const tffStore = db.createObjectStore(
                        that.objectStoreNameFor(
                            CFTCReportType.FinancialFutures
                        ),
                        { keyPath: 'id' }
                    )
                    tffStore.createIndex(
                        'cftc_contract_market_code_idx',
                        'cftc_contract_market_code',
                        { unique: false }
                    )
                    tffStore.createIndex('timestamp_idx', 'timestamp', {
                        unique: false,
                    })

                    // Disaggregated (Agriculture & Natural Resources) ObjectStore
                    const disaggregatedStore = db.createObjectStore(
                        that.objectStoreNameFor(CFTCReportType.Disaggregated),
                        { keyPath: 'id' }
                    )
                    disaggregatedStore.createIndex(
                        'cftc_contract_market_code_idx',
                        'cftc_contract_market_code',
                        { unique: false }
                    )
                    ...
                    // Legacy ObjectStore
                    const legacyFuturesReportsStore = db.createObjectStore(
                        that.objectStoreNameFor(CFTCReportType.Legacy),
                        { keyPath: 'id' }
                    )
                    legacyFuturesReportsStore.createIndex(
                        'cftc_contract_market_code_idx',
                        'cftc_contract_market_code',
                        { unique: false }
                    )
                    ...
                    const contractTypesStore = db.createObjectStore(
                        'commodityContracts',
                        { keyPath: 'cftcContractMarketCode' }
                    )
                    // rather than making a new `ObjectStore` for each report type, each entry is a tagged struct with `reportType`:
                    contractTypesStore.createIndex(
                        'report_type_idx',
                        'reportType',
                        { unique: false }
                    )
                    contractTypesStore.createIndex(
                        'commodity_group_name_idx',
                        'group',
                        { unique: false }
                    )
                    contractTypesStore.createIndex(
                        'cftc_commodity_code_idx',
                        'cftcCommodityCode',
                        { unique: false }
                    )
                }
    ...
    public async requestDateRange(
        request: DateRangeRequest
    ): Promise<IAnyCOTReportType[]> {
        const cached: IAnyCOTReportType[] =
            await this.getCachedDateRange(request)
        ...
        let dst = [...cached]
        dst.sort(
            (a: IAnyCOTReportType, b: IAnyCOTReportType) =>
        ...
        if (
            request.startDate.getTime() < oldest['timestamp'] &&
            ...
            if (missingOlderData != null && missingOlderData.length === 0) {
                ...
                missingOlderData != null &&
                missingOlderData.length > 0
            ) {
                await this.storeFuturesRecords(
                    request.reportType,
                    missingOlderData
                )
                missingOlderData.sort(
        ...
        if (request.endDate.getTime() > youngest['timestamp']) {
            ...
            const daysBetween = daysDiff(
                request.endDate,
                youngestReportDateTime
            )
    ...
    private async getCachedDateRange(
        request: DateRangeRequest
    ): Promise<IAnyCOTReportType[]> {
        const db = await this.dbHandle
        return new Promise((resolve, reject) => {
            ...
            req.onsuccess = (ev) => {
                const cursor = (ev.target as IDBRequest<IDBCursorWithValue>)
                    .result
                if (cursor != null) {
                    if (
                        cursor.value['cftc_contract_market_code'] ===
                            request.cftcContractMarketCode &&
    ...
    private async storeFuturesRecords(
        reportType: CFTCReportType,
        payload: IAnyCOTReportType[]
    ): Promise<void> {
        const db = await this.dbHandle
        const objectStoreName = this.objectStoreNameFor(reportType)
        return new Promise((resolve, reject) => {
            ...
            for (const record of payload) {
                ...
                if (record == null || !record.hasOwnProperty('timestamp')) {
                    reject(
                        new Error('futures report item was not preprocessed')
                    )
                }
                const txReq = objectStore.put(record)
                txReq.onsuccess = (ev) => {
                    const insertedKey = (ev.target as IDBRequest<IDBValidKey>)
                        .result
                    if (insertedKey !== record['id']) {
                        reject(
                            new Error(
                                'assertion failure: IndexedDB did not insert key as expected'
                            )
                        )
                    }
                }
        ...
        return new Promise((resolve, reject) => {
            ...
            for (const entry of payload) {
                ...
                insertRequest.onsuccess = (ev) => {
                    const validKey = (ev.target as IDBRequest<IDBValidKey>)
                        .result
    ...
    private async getCachedContracts(
        request: ContractListRequest
    ): Promise<CommodityContractKind[]> {
        const db = await this.dbHandle
        return new Promise((resolve, reject) => {
