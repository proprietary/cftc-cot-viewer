...
function rollingMovingAverage(a: ArrSlice<number>, lookback: number): number[] {
    ...
    for (
        ;
        subIndex < a.endIndex && subIndex - a.startIndex < lookback;
        ++subIndex
    ) {
...
function rollingStdDev(
    ...
    LHAssert(
        aMovingAvg.length === dst.length || aMovingAvg.length === a.arr.length
    )
    let subVariance = 0
    let subIndex = a.startIndex
    for (
    ...
    for (; subIndex < a.endIndex; ++subIndex) {
        ...
        subVariance -= Math.pow(
            a.arr[subIndex - lookback] - aMovingAvg[subIndex - lookback],
            2
        )
        subVariance += Math.pow(a.arr[subIndex] - aMovingAvg[subIndex], 2)
...
export function rollingMinMaxScaler(
    arr: Array<number>,
    lookback: number,
    scaleMin: number = -1.0,
    scaleMax: 1.0
...
export function rollingMinMaxScalerOptimized(
    arr: number[],
    lookback: number,
    scaleMin: number = -1.0,
    scaleMax: number = 1.0
    ...
    for (let idx = 0; idx < arr.length; ++idx) {
        ...
        if (excludedIndex === rollingMin) {
            ...
            rollingMin = indexWithMin({
                arr,
                startIndex: excludedIndex + 1,
                endIndex: idx + 1,
            })
        }
        if (excludedIndex === rollingMax) {
            // recompute max without the excluded item to the left of this lookback window
            rollingMax = indexWithMax({
                arr,
                startIndex: excludedIndex + 1,
                endIndex: idx + 1,
            })
        ...
        let std =
            (arr[idx] - arr[rollingMin]) / (arr[rollingMax] - arr[rollingMin])
        let scaled = std * (scaleMax - scaleMin) + scaleMin
        dst[idx] = scaled
    }
    return dst
...
export function rollingRobustScaler(
    a: Array<number>,
    lookback: number
): Array<number> {
...
function arrSliceInterQuartileRange(
    a: ArrSlice<number>
): [q1: number, q3: number] {
    let arr = a.arr.slice(a.startIndex, a.endIndex).sort((a, b) => a - b)
    let q1 = arrSliceMedian(
        { arr, startIndex: 0, endIndex: Math.floor(arr.length / 2) },
        true
    )
    let q3 = arrSliceMedian(
        { arr, startIndex: Math.ceil(arr.length / 2), endIndex: arr.length },
        true
    )
    return [q1, q3]
}

function arrSliceMedian(
    a: ArrSlice<number>,
...
function quantileTransform(
    a: ArrSlice<number>,
    scaleLow: number = -1.0,
    scaleHigh: number = 1.0
): number[] {
    const ranked = a.arr
        .slice(a.startIndex, a.endIndex)
        .map((value, originalIndex) => [value, originalIndex])
        .sort(
            ([aValue, aOriginalIndex], [bValue, bOriginalIndex]) =>
                aValue - bValue
        )
    ...
    for (let i = a.startIndex; i < a.arr.length && i < a.endIndex; ++i) {
        ...
        let percentileInScale =
            scaleLow + (scaleHigh - scaleLow) * rawPercentile
        transformed.push(percentileInScale)
    }
    return transformed
}

export function quantileTransformAt(
    a: ArrSlice<number>,
    atIndex: number,
    scaleLow: number = -1.0,
    scaleHigh: number = 1.0
...
export function rollingQuantileNormalization(
    arr: Array<number>,
    lookback: number
): number[] {
    let dst: number[] = []
    for (let i = 0; i < arr.length; ++i) {
        ...
        let intermediateResult = quantileTransformAt(
            { arr, startIndex, endIndex: i + 1 },
            i - startIndex
        )
