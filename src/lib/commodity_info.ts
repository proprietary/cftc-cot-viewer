...
export class CommodityInfoService {
    ...
    public async requestPriceFeed(
        ...
        let priceBars = await this.cache.getPriceBars(
            priceFeed.source,
            cftcCommodityCode
        )
        if (
            priceBars.length === 0 ||
            priceBars.at(-1)!.timestamp.getTime() < lookUpToThisDate.getTime()
        ) {
            const newPriceBars = await this.requestExternalPriceFeed(
                priceFeed,
                lookUpToThisDate
            )
            if (newPriceBars.length > 0) {
                priceBars = priceBars.concat(newPriceBars)
                await this.cache.putPriceBars(
                    priceFeed.source,
                    cftcCommodityCode,
                    priceBars
                )
    ...
    private selectRelevantPriceBars(
        priceBars: PriceBar[],
        onlyOnDates: readonly Date[]
    ): PriceBar[] {
        let priceBarsIdx = 0
        let relevantPriceBars: PriceBar[] = onlyOnDates.map((timestamp) => {
            ...
            while (
                tempPriceBarsIdx < priceBars.length &&
                !sameDay(priceBars[tempPriceBarsIdx].timestamp, timestamp)
            ) {
                ++tempPriceBarsIdx
            }
            if (
    ...
    private async requestExternalPriceFeed(
        priceFeed: IPriceFeed,
        since?: Date,
        until?: Date
    ): Promise<PriceBar[]> {
        switch (priceFeed.source) {
            case 'FRED': {
                let series: PriceBar[] = []
                try {
                    series = await requestFredObservations(
                        priceFeed.symbol,
                        since!
                    )
                } catch (e) {
...
export class CommodityInfoDatabase {
    ...
    private idb(): Promise<IDBDatabase> {
        ...
        return new Promise((resolve, reject) => {
            ...
            let idbOpenRequest = window.indexedDB.open(
                'so_libhack_cot_commodity_info',
                1
            )
            idbOpenRequest.onupgradeneeded = (ev) => {
                ...
                switch (ev.oldVersion) {
                    case 0: {
                        ...
                        const priceBarsStore = db.createObjectStore(
                            'priceBars',
                            {
                                keyPath: [
                                    'cftcCommodityCode',
            ...
            idbOpenRequest.onsuccess = (ev) => {
                ...
                db.onversionchange = () => {
                    db.close()
                    window.alert(
                        'App updated in the background. Please refresh the page.'
                    )
    ...
    public getPriceBars(
        ...
        return this.idb().then((db: IDBDatabase) => {
            return new Promise((resolve, reject) => {
                ...
                const priceBarsStore = txn.objectStore('priceBars')
                const request = priceBarsStore.get(
                    IDBKeyRange.only([cftcCommodityCode, source])
                )
                request.onsuccess = (ev) => {
                    const result = (ev.target as IDBRequest<any>).result
                    resolve(
                        result == null || result['bars'] == null
                            ? []
                            : (result['bars'] as PriceBar[])
                    )
                }
                request.onerror = (ev) => {
...
export function alignDatetimes<T, Q>(
    src: DatetimeAlignmentContainer<T>,
    fitToTarget: DatetimeAlignmentContainer<Q>
): T[] {
    ...
    const targetData = [...fitToTarget.items].sort(
        (a: Q, b: Q) =>
            fitToTarget.getDate(b).getTime() - fitToTarget.getDate(a).getTime()
    )
    let srcData = [...src.items].sort(
        (a: T, b: T) => src.getDate(b).getTime() - src.getDate(a).getTime()
    )

    // using the `fitToTarget` dates, look for the entries in `src` with nearest dates
    for (
        let targetIdx = 0, srcIdx = 0;
        srcIdx < srcData.length && targetIdx < targetData.length;
        ++targetIdx
    ) {
        ...
        for (let srcIdx2 = srcIdx; srcIdx2 < srcData.length; ++srcIdx2) {
            let maybeMinimalTimeDiff = Math.abs(
                src.getDate(srcData[srcIdx2]).getTime() - targetDate.getTime()
            )
